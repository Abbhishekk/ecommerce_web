    ...
    return (
        <div className=' flex justify-between w-auto z-40 items-center mr-10 ml-10 mt-2 pb-2 '>
            <div className="nav-logo md:flex hidden items-center justify-center">
                <Link to={'/'} reloadDocument className='nav-logo md:flex hidden items-center justify-center' >
            ...
            <div className='md:hidden block cursor-pointer hover:text-gray-500'>
                ...
                {hamburger && <div>
                    <ul className="md:hidden gap-5 md:text-lg font-semibold absolute bg-white w-56 py-6 px-3  transition ease-in-out flex-col">
                        <li className='border-b-2 border-orange-500 cursor-pointer hover:text-gray-500 m-5'> <Link reloadDocument to={"/"} > Shop</Link></li>
                        <li className='border-b-2 border-orange-500 cursor-pointer hover:text-gray-500 m-5'><Link reloadDocument to={"/men"} >Men </Link> </li>
                        <li className='border-b-2 border-orange-500 cursor-pointer hover:text-gray-500 m-5'><Link reloadDocument to={"/women"} >Women </Link></li>
                        <li className='border-b-2 border-orange-500 cursor-pointer hover:text-gray-500 m-5'><Link reloadDocument to={"/kids"} >Kids </Link></li>
                    </ul>

                </div>}
            </div>
            <ul className="md:flex gap-5 text-lg font-semibold  transition ease-in-out hidden">
                <li className='hover:border-b-2 hover:border-orange-500 cursor-pointer hover:text-gray-500'> <Link reloadDocument to={"/"} > Shop</Link></li>
                <li className='hover:border-b-2 hover:border-orange-500 cursor-pointer hover:text-gray-500'><Link reloadDocument to={"/men"} >Men </Link> </li>
                <li className='hover:border-b-2 hover:border-orange-500 cursor-pointer hover:text-gray-500'><Link reloadDocument to={"/women"} >Women </Link></li>
                <li className='hover:border-b-2 hover:border-orange-500 cursor-pointer hover:text-gray-500'><Link reloadDocument to={"/kids"} >Kids </Link></li>
            </ul>
            ...
            <div className='flex gap-5 items-center'>
                <div>
                    <Link to={"/cart"} reloadDocument className='relative'>
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6 h-6">
                            <path strokeLinecap="round" strokeLinejoin="round" d="M15.75 10.5V6a3.75 3.75 0 1 0-7.5 0v4.5m11.356-1.993 1.263 12c.07.665-.45 1.243-1.119 1.243H4.25a1.125 1.125 0 0 1-1.12-1.243l1.264-12A1.125 1.125 0 0 1 5.513 7.5h12.974c.576 0 1.059.435 1.119 1.007ZM8.625 10.5a.375.375 0 1 1-.75 0 .375.375 0 0 1 .75 0Zm7.5 0a.375.375 0 1 1-.75 0 .375.375 0 0 1 .75 0Z" />
                        </svg>
                        <p className='absolute mt-[-35px] ml-[15px] bg-orange-500 px-1 text-white rounded-full text-xs' >{(cart.length)}</p>
                    </Link>
                </div>
                <p>
                    {
                        user == null && (
                            <button className='border-2 border-orange-500 px-3 py-1  transition ease-in-out   rounded hover:bg-orange-500 hover:text-white' > <Link to={"/login"} reloadDocument > Login </Link></button>
                            ...
                            <Menu>
                                <MenuHandler>
                                    <Avatar
                                        ...
                                        src={user?.avatar} placeholder={undefined} />
                                </MenuHandler>
                                <MenuList placeholder={undefined}>
                                    ...
                                    <MenuItem className="flex items-center gap-2 " placeholder={undefined}>
                                        ...
                                        <Typography variant="small" className="font-medium" placeholder={undefined} onClick={handleClick}>
